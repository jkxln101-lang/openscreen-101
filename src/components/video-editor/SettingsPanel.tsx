...
const MP4_EXPORT_SHORT_SIDES: Record<ExportQuality, number | null> = {
	medium: 720,
	good: 1080,
	high: 1440,
	ultra: 2160,
...
const EXPORT_QUALITY_OPTIONS: { value: ExportQuality; labelKey: string }[] = [
	...
	{ value: "high", labelKey: "exportQuality.high" },
	{ value: "ultra", labelKey: "exportQuality.ultra" },
	{ value: "source", labelKey: "exportQuality.source" },
];
...
export function SettingsPanel({
	...
	const [isExportQualityOpen, setIsExportQualityOpen] = useState(false);
	const exportQualityButtonRef = useRef<HTMLButtonElement>(null);
	const exportQualityDropdownRef = useRef<HTMLDivElement>(null);
	const sourceDimensions = formatSourceDimensions(videoElement, cropRegion);

	// Close export quality dropdown when clicking outside
	useEffect(() => {
		if (!isExportQualityOpen) return;
		const handleClickOutside = (event: MouseEvent) => {
			const target = event.target as Node;
			const isClickInsideButton = exportQualityButtonRef.current?.contains(target);
			const isClickInsideDropdown = exportQualityDropdownRef.current?.contains(target);

			if (!isClickInsideButton && !isClickInsideDropdown) {
				setIsExportQualityOpen(false);
			}
		};
		// Use setTimeout to avoid catching the mousedown that opened the dropdown
	...
	return (
		<>
							...
							<div className="editor-panel-section mb-3 space-y-3 px-1">
								...
								{zoomEnabled && (
									<div className="flex items-center gap-2">
										<Button
											type="button"
											onClick={() => onRegionSelectionModeChange?.(!regionSelectionMode)}
											className={cn(
												"h-8 flex-1 rounded-lg border px-2 text-center transition-all duration-150 ease-out",
												"cursor-pointer",
												regionSelectionMode
													? "border-[#34B27B]/70 bg-[#34B27B] text-white shadow-[0_8px_20px_rgba(52,178,123,0.18)]"
													: "border-white/[0.06] bg-white/[0.035] text-slate-400 hover:bg-white/[0.075] hover:border-white/15 hover:text-slate-200",
							...
							{exportFormat === "mp4" && (
								<div className="mb-3 space-y-1.5">
									...
									<div className="relative">
										<button
											ref={exportQualityButtonRef}
											type="button"
											onClick={() => setIsExportQualityOpen((open) => !open)}
											className="flex h-9 w-full items-center justify-between rounded-lg border border-white/10 bg-white/[0.045] px-3 text-[11px] text-white/85 transition-colors hover:bg-white/10"
										>
											<span>
												{EXPORT_QUALITY_OPTIONS.find((o) => o.value === exportQuality)
													? t(
															EXPORT_QUALITY_OPTIONS.find((o) => o.value === exportQuality)!
																.labelKey,
														)
													: ""}
												{exportQuality === "source" && sourceDimensions
			...
			{isExportQualityOpen &&
				createPortal(
					<div
						ref={exportQualityDropdownRef}
						className="fixed z-[100] mt-1 overflow-y-auto rounded-lg border border-white/10 bg-[rgba(8,9,12,0.96)] p-1 shadow-[0_18px_42px_rgba(0,0,0,0.48)] backdrop-blur-[18px]"
						style={{
							left: exportQualityButtonRef.current?.getBoundingClientRect().left,
							top: (exportQualityButtonRef.current?.getBoundingClientRect().bottom ?? 0) + 4,
							width: exportQualityButtonRef.current?.getBoundingClientRect().width,
						}}
						onClick={(e) => e.stopPropagation()}
					>
						{EXPORT_QUALITY_OPTIONS.map((option) => {
							const shortSide = MP4_EXPORT_SHORT_SIDES[option.value];
							const isUpscale =
								shortSide !== null && sourceDimensions && sourceDimensions.shortSide < shortSide;
							const label = t(option.labelKey);
							const suffix =
								option.value === "source" && sourceDimensions
									? ` (${sourceDimensions.shortSide}p)`
									: isUpscale
										? ` — ${t("exportQuality.upscale")}`
										: "";
							const isSelected = option.value === exportQuality;
							return (
								<button
									key={option.value}
									type="button"
									...
									className={`flex w-full items-center justify-between rounded-md px-3 py-2 text-[11px] transition-colors ${
										isSelected
											? "bg-[rgba(52,178,123,0.14)] text-white"
								...
								>
									...
									{isSelected && <span className="text-[#34B27B]">✓</span>}
								</button>
							);
						})}
					</div>,
